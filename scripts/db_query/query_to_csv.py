#!/usr/bin/env python3
"""
executeQuery API 결과(JSON)를 CSV 파일로 저장하는 스크립트.

사용 예시:
  python query_to_csv.py --sql "SELECT * FROM HDEL_DEFAULT.SOME_TABLE" --output result.csv
  python query_to_csv.py --sql-file query.sql --output result.csv

사내 인증서 문제로 SSL 오류가 발생할 때만:
  python query_to_csv.py --sql-file query.sql --output result.csv --insecure
"""
...
def find_table_payload(payload: Any) -> tuple[list[str], list[Any]]:
    ...
    raise ValueError("CSV로 변환할 데이터 목록을 찾지 못했습니다.")
...
def rows_to_csv_records(headers: list[str], rows: list[Any]) -> tuple[list[str], list[dict[str, Any]]]:
    if not rows:
        return headers, []
...
def write_csv(path: str, headers: list[str], records: list[dict[str, Any]]) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="executeQuery API를 GET 방식으로 호출하고 JSON 결과를 CSV로 저장합니다."
    )
    parser.add_argument("--sql", help="실행할 SQL 문장")
    parser.add_argument("--sql-file", help="SQL이 저장된 파일 경로")
    parser.add_argument("--output", default="result.csv", help="저장할 CSV 파일명")
    parser.add_argument(
        "--insecure",
        action="store_true",
...
def main() -> int:
    try:
        ...
        headers, records = rows_to_csv_records(headers, rows)

        if not headers:
            raise ValueError("CSV 헤더를 만들 수 없습니다.")

        write_csv(args.output, headers, records)
        print(f"CSV 저장 완료: {args.output} / {len(records)}건")
        return 0
