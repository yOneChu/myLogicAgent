...
def main():
    ...
    print("=== Execution Trace for PID EL_PB128A01 (HOGI: 213249L36) ===")
    ...
    while idx < len(rows):
        ...
        print(f"Row {str(no):>3s} | ADDR: {addr:12s} | GOTO: {goto:10s} | Status: {status:7s}{out_str} | Remarks: {remarks}")
        if not matched and (addr != '-' or goto != '-'):
            print(f"   --> Mismatch Reason: {reasons[0] if reasons else 'N/A'}")
