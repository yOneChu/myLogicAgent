...
- API 호출을 위한 기본 베이스 URL : 'https://vault-in.hdel.co.kr:8070/api/'로 시작한다.
...
## PID (수배로직 PID) 메타 정보 조회 API
LLM 및 에이전트가 PID (수배로직 PID) 정보 추출 및 SQL 쿼리를 생성하기 위해 참고하는 **메타데이터 정의서**를 조회합니다.

- **URL**: `https://vault-in.hdel.co.kr:8070/api/getPIDMetaInfo`
- **HTTP Method**: `GET`
- **파라미터**:
  | 파라미터명 | 타입 | 필수 여부 | 설정값 | 설명 |
  |---|---|---|---|---|
...
- **호출 예시**:
  ```text
  GET https://vault-in.hdel.co.kr:8070/api/getPIDMetaInfo?key=subae
  ```
